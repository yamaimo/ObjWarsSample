...
from terminal import Terminal, Terminal_put_empty_line, Terminal_put_str
...
def Game_init(deal: Deal, player0: Player, player1: Player, terminal: Terminal) -> Game:
    """ゲームを生成して返す"""
...
def Game_start(game: Game) -> Player:
    ...
    while True:
        ...
        available_actions = ActionList_get_available_actions(turn_hand, prev_action)
        action = Player_select_action(turn_player, available_actions)

        if Action_is_ask(action):
            ask_action = cast(AskAction, action)
            card_number = Card_get_number(AskAction_get_card(ask_action))
            Terminal_put_str(terminal, f"{turn_player_name} asked {card_number}")
            ...
            card_number = Card_get_number(GuessAction_get_card(guess_action))
            Terminal_put_str(terminal, f"{turn_player_name} guessed {card_number}")
...
if __name__ == "__main__":
    ...
    from player import HumanPlayer_init, Player_init_with_human_player
    from terminal import Terminal_init

    dealer = Dealer_init(0)
    deal = Dealer_deal(dealer)

    terminal = Terminal_init(in_stream=StringIO("ask 2\nask 3\nguess 4\n"))
    human0 = HumanPlayer_init("player0", Deal_get_player0_hand(deal), terminal)
    human1 = HumanPlayer_init("player1", Deal_get_player1_hand(deal), terminal)
